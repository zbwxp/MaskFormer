...
class HungarianMatcher_iou_reduction(nn.Module):
    ...
    def memory_efficient_forward(self, outputs, targets, _iter=None):
        ...
        for b in range(bs):
            ...
            # iou reduction
            self.matrix_nms_reduction(out_mask)
    ...
    def matrix_nms_reduction(self, seg_masks):
        n_samples = len(seg_masks)
        if n_samples == 0:
            print("zero samples left!!!!! something is wrong!!!!")
            assert False
        seg_masks = seg_masks.sigmoid()
        # conf_score = (seg_masks > 0.9).sum((1, 2)) / (seg_masks > 0.3).sum((1, 2))
        sum_masks = (seg_masks > 0.5).sum((1, 2))
        ...
        iou_max, idx = iou_matrix.max(1)
        # keep = torch.zeros_like(conf_score) + 1
        fuse = []
        possible_overlap = (iou_matrix > 0.65).nonzero(as_tuple=False)
        # collect similar masks together
        for i in range(n_samples):
            pairs = possible_overlap[possible_overlap[:, 0] == i].flatten().unique()
            if len(pairs) > 0:
                fuse.append(pairs)
        ...
        for idx in possible_overlap[:, -1].unique():
            if (possible_overlap[:, -1] == idx).sum() == 1:
                # two masks group
                fuse.append(possible_overlap[possible_overlap[:, -1] == idx][0])
            else:
                # multiple masks group
                pairs = possible_overlap[possible_overlap[:, -1] == idx].flatten().unique()
                fuse.append(pairs)
