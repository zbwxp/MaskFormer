...
class MaskFormer_seperatev2(nn.Module):
    """
    ...
    def forward(self, batched_inputs):
        ...
        if self.training:
            if not self.use_gt_targets:
                ...
                labels = torch.zeros(pred_cls_logits.size()[0:2], device=self.device).long() + 150
                for i, indice in enumerate(indices):
                    labels[i][indice[0]] = targets[i]['labels'][indice[1]]

                labels = torch.as_tensor(labels).flatten()
                loss_ce_cls = F.cross_entropy(pred_cls_logits.reshape(-1, self.num_classes + 1), labels)
                losses.update({"loss_cls_by_pred": loss_ce_cls})
